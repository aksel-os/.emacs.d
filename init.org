...
(defun suppress-lexical-binding-warnings (warn)
  "Suppress warnings about missing lexical-binding cookies."
  (if (string-match "Missing ‘lexical-binding’ cookie" (car warn))
      (setq suppress-without-warning t) ; suppress the warning
    nil))

(add-hook 'warning-filter-hook 'suppress-lexical-binding-warnings)
#+end_src

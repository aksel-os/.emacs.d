...
This allows me to specify things such as lsp paths in =.dir-locals.el= on remote
servers, such that Eglot works there aswell.
...
(use-package auto-dark
  ...
  (setq auto-dark-polling-interval-seconds 1))
#+end_src
...
(use-package corfu-popupinfo
  ...
  (custom-set-faces
   '(corfu-popupinfo ((t (:inherit default))))
   '(corfu-popupinfo-buffer ((t (:inherit default))))))
#+end_src
...
** Eldoc Box
   ...
   (use-package eldoc-box
     ...
     :config
     (setq eldoc-box-hover-render-function #'identity)
     (setq eldoc-box-max-width 80
           eldoc-box-max-height 30))
   ...
   (with-eval-after-load 'eldoc-box
     ...
     ;; Doommodeline fuckery
     (with-eval-after-load 'eldoc-box
       (add-hook 'eldoc-box-hover-at-point-mode-hook
                 (lambda ()
                   (if eldoc-box-hover-at-point-mode
                       (setq-local eldoc-message-function #'eldoc-box--eldoc-message)
                     (kill-local-variable 'eldoc-message-function)))))
...
** Kotlin
   #+begin_src emacs-lisp
      (use-package kotlin-mode
        ...
        (kotlin-mode . eglot-format-on-save)
        :config
        (add-to-list 'eglot-server-programs
...
** PostgreSQL
   ...
   (use-package sqlformat
     ...
     (setq sql-set-dialect 'postgres)
     (setq sqlformat-args '("M" "-p '-- pgfmt: off(?:.*)?-- pgfmt: on'"))
     )
   #+end_src

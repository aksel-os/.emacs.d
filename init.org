...
* Start
All this code is taken from [[https://github.com/SophieBosio/.emacs.d/tree/main][Sophie Bosio]].
...
As stated earlier I mainly use nix on my systems, but nix on windows is I do *not* want to experience, so the following are the package sources.
...
* Visual
** Decluttering
This is added to =early-init.el=.
...
I'm currently testing this to see if there really is a difference, this also goes in the =early-init.el=.
...
** Theme
I use [[https://github.com/catppuccin/emacs][catppuccin]] as my theme, with the frappe theme as my default.
...
I my current preferred font is [[https://www.jetbrains.com/lp/mono/][jetbrains mono]].

#+begin_src emacs-lisp
  (defvar akselos/font-height 140)
    
    (set-face-attribute 'default nil :height akselos/font-height)

  (when (member "JetBrains Mono" (font-family-list))
    (set-face-attribute 'default nil :font "JetBrains Mono" :height akselos/font-height)
#+end_src
...
* General Editing
** Built-ins
This is copied for Lars Tveito's repo, aswell as from the emacs course he had.
...
#+end_src
  ...
  Some variables er buffer-local, so we have to change them using =setq-default=.
...
* Buffers and Frames
** Killing Buffers
Taken from this [[https://superuser.com/questions/895920/how-can-i-close-all-buffers-in-emacs][StackExchange]] post. It seemed interessting. =C-c k= kills the current buffer, whilst =C-u C-c k= kills all external Emacs buffers.
...
** Corfu
I use [[https://github.com/minad/corfu][corfu]] for =completion-in-region=. I wish to have a VSCode like completion ui, therefore I have enabled =corfu-auto=. Perhaps one day I get proficient with =M-\=.
...
* Org
Org mode is extremely nice for note-taking, and I use it every day.
...
** LaTeX export
[[https://github.com/tecosaur/engrave-faces][engrave-faces]] syntax highlights source blocks after your color theme.
...
** Java
Abbreviations for the most tedious Java stuffs
...
To use the abbrev table, =abbrev-mode= must be activated.
